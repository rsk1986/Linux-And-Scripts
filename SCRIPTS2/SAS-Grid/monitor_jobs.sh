...
do
        ...
        op3=`echo $jobtp | fgrep 'Project' | wc -l`
...
#       echo $dif
        ...
        then
                ...
                then
                        ...
                        /sascommon/SAS/LSFHome/10.1/linux2.6-glibc2.3-x86_64/bin/bkill $j1
                elif [ $op2 == 1 ]
                then
                        #echo "$job -> Web Infra Platform Services"
                        echo -e "$j1 \t $j2 \t $j3 \t $j6 \t $j5 \t $j4 \t $j7 \t Web Infra Platform Services" >> /tmp/monitor_jobs2.tmp
                        /sascommon/SAS/LSFHome/10.1/linux2.6-glibc2.3-x86_64/bin/bkill $j
                elif [ $op3 == 1 ]
                then
                #echo "$job -> SASGSUB"
...
mail -s "SAS Grid - Val Prod - Terminated Long Running Jobs( > 3 days )" $mail_id < /tmp/monitor_jobs.tmp
